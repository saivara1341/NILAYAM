                        ...
                        return (
                            ...
                            >
                                <div className={`
                                    ...
                                    ${isActive ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-neutral-400 dark:text-neutral-500'}
                                `}>
                                    {React.cloneElement(item.icon as React.ReactElement<any>, { 
                                        className: `w-6 h-6 transition-all duration-300 ${isActive ? 'stroke-[2.5px]' : 'stroke-2'}` 
                                    })}
                                </div>
                                <span className={`text-[10px] font-medium tracking-wide transition-all duration-300 ${isActive ? 'text-blue-600 dark:text-blue-400 translate-y-0 opacity-100' : 'text-neutral-400 dark:text-neutral-500 opacity-80'}`}>
                                    {t(item.labelKey)}
                                </span>
                            </NavLink>
                        );
